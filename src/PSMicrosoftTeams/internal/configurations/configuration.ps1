...
Set-PSFConfig -Module $script:ModuleName -Name 'Settings.GraphApiQuery.Select.Cahnnel' -Value @('id','displayName','description','isFavoriteByDefault','email','membershipType') -Initialize  -Validation 'stringarray' -Description "Specifies filter select parameter to return a set of properties for a conversation member resource from Microsoft Graph API, including mail address if available."
