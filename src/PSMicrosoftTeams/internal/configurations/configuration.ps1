...
Set-PSFConfig -Module 'PSMicrosoftTeams' -Name 'Settings.PostContentType' -Value 'application/json' -Initialize -Validation 'string' -Description "Specifies post content type of rest method."
...
Set-PSFConfig -Module 'PSMicrosoftTeams' -Name 'Settings.GraphApiQuery.Select.Group' -Value @('id', 'createdDateTime','displayName','description','mail','mailNickName','mailEnabled','visibility') -Initialize -Validation 'stringarray' -Description "Specifies uery parameter to return a set of properties that are different than the default set for an individual resource or a collection of resources Get-PSMTTeam."
