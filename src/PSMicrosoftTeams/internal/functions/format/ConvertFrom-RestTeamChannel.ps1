function ConvertFrom-RestTeamChannel {
	<#
	.SYNOPSIS
	...
	.EXAMPLE
		PS C:\> Invoke-RestRequest -Service 'graph' -Path users -Query $query -Method Get -ErrorAction Stop | ConvertFrom-RestTeamChannel
	...
	$type = if ($InputObject -is [array]) {
		[PSMicrosoftTeams.Channels.Channel[]]
	}
	else {
		[PSMicrosoftTeams.Channels.Channel]
	}
