function Connect-PSMicrosoftTeams {
	...
	.PARAMETER MakeDefault
		Makes this service the new default service for all subsequent Connect-PSMicrosoftTeams & Invoke-EntraRequest calls.
	...
	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -ClientID $clientID -TenantID $tenantID
	
		Establish a connection to the graph API, prompting the user for login on their default browser.

	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -AsAzAccount

		Establish a connection to the graph API, using the current Az.Accounts session.
	
	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -ClientID $clientID -TenantID $tenantID -Certificate $cert
	
		Establish a connection to the graph API using the provided certificate.
	
	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -ClientID $clientID -TenantID $tenantID -CertificatePath C:\secrets\certs\mde.pfx -CertificatePassword (Read-Host -AsSecureString)
	...
	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -Service Endpoint -ClientID $clientID -TenantID $tenantID -ClientSecret $secret
	
		Establish a connection to Defender for Endpoint using a client secret.
	
	.EXAMPLE
		PS C:\> Connect-PSMicrosoftTeams -ClientID $clientID -TenantID $tenantID -VaultName myVault -Secretname GraphCert
