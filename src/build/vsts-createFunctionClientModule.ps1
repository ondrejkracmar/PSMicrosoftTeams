...
$parentModule = 'PSMicrosoftTeams'
if (-not $ModuleName) { $ModuleName = 'PSMicrosoftTeams.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSMicrosoftTeams'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSMicrosoftTeams.psd1"
$prereqHash = @{
